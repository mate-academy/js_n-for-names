...
function nIsForName(names) {
  const arrNisForName = names.filter(function(elem) {
    const conditionNisForName = elem.split(' ');
    return conditionNisForName.some(function(str) {
      return str[0].includes('N');
    });
  });
  return arrNisForName;
}
